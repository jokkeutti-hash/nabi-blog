...
8. **다양성 강제 규칙 ⚠️ 반드시 준수**: 아래 두 가지 다양성을 동시에 지켜야 합니다.
   ...
   **[주제 다양성 — 카테고리 탈피 필수]**
   - 10개 중 **최대 4개**만 이 블로그의 현재 주요 카테고리 내 주제로 채울 수 있습니다.
   - 나머지 **6개 이상**은 현재 블로그 카테고리에서 벗어난 **인접·확장·완전히 다른 분야**에서 가져와야 합니다.
   - 예시:
     - 건강 블로그 → 인접: 뷰티·피부관리, 심리·멘탈, 식품 레시피 / 확장: 여행, 재테크, 라이프스타일 트렌드
     - IT 블로그 → 인접: 업무 생산성, AI 활용법 / 확장: 스마트홈, 전기차, 취미 테크
     - 여행 블로그 → 인접: 맛집·카페, 숙박 후기 / 확장: 여행 준비 꿀팁, 국내 이색 체험, 여행 재테크
     - 그 외도 동일하게 주요 카테고리 **밖**의 트렌드 주제를 적극 포함
   - 같은 하위 분야 주제 2회 이상 반복 금지 (카테고리 내·외 모두 적용)
